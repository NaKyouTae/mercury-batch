...
public class UserService {
	...
	public UserService() {
		try {
			this.threeRepository = ApplicationContextProvider
					.getBean("threeRepository");
			this.twoRepository = ApplicationContextProvider
					.getBean("twoRepository");
			this.wordRepository = ApplicationContextProvider
					.getBean("wordRepository");
			this.gradeRepository = ApplicationContextProvider
					.getBean("gradeRepository");
			this.userGradeRepository = ApplicationContextProvider
					.getBean("userGradeRepository");
		} catch (Exception e) {
	...
	public <T extends Object> T upUserGrade(User user) throws Exception {
		UserGrade userGrade = userGradeRepository
				.findByUserName(user.getUsername());

		Grade grade = gradeService
				.getByStartRangeLessThanAndEndRangeGreaterThan(
						user.getMileage());
	...
	public <T extends Object> T upUserThreePoint() throws Exception {
		...
		List<Three> three = threeRepository.findByWordIdx(word.getIdx(),
	...
	public <T extends Object> T upUserTwoPoint() throws Exception {
		...
		List<Two> two = twoRepository.findByWordIdx(word.getIdx(),
				Sort.by(Sort.Direction.DESC, "point"));
