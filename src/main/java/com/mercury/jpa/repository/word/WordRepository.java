...
public interface WordRepository extends JpaRepository<Word, Long> {
	...
	@Query(value="select idx, wordgroup, word, insertdate, startdate, enddate, description from mercury_tb_word where endDate >= date(now()) and wordGroup = :group ORDER BY endDate desc", nativeQuery = true)
	List<Word> findByWordGroup(@Param("group")  String group);
